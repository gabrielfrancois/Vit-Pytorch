...
def load_imagenet1k(
    ...
    """
    Load the dataset ImageNet-1k (version 128x128 on Hugging Face)
    """

    print("Loading dataset: ImageNet-1k (benjamin-paine/imagenet-1k-128x128)...")
    ...
    ds_train = load_dataset("benjamin-paine/imagenet-1k-128x128",
        split="train[:{}]".format(max_items_train) if max_items_train else "train",
        cache_dir=cache_dir
    )
    ds_val = load_dataset(
