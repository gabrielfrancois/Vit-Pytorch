...
import numpy as np
import torch
from torch.utils.data import (  # maybe we should avoid random split to make it more reproducible
...
def load_CIFAR(data_dir: str, CIFAR: int) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    test_transform = T.Compose(
        [  # absolutely no data augmentation here!
            T.Resize((32, 32)),
            T.ToTensor(),
        ]
    )
    ...
    if CIFAR == 10:
        # Train data
        train_batches = [
            unpickle(os.path.join(data_dir, f"data_batch_{i}")) for i in range(1, 5 + 1)
        ]
        ...
        # build datasets
        train_transform = T.Compose(
            [T.ToPILImage(), train_transform]
        )  # necessary for CIFAR 10
        test_transform = T.Compose([T.ToPILImage(), test_transform])
        full_train_dataset = CIFAR_Datasets(
            CIFAR=10, X=X_train, y=y_train, transform=train_transform
        )
        test_dataset = CIFAR_Datasets(
            CIFAR=10, X=X_test, y=y_test, transform=test_transform
        )
...
def visualise(loader, output_dir):
    ...
    output_path = os.path.join(
        output_dir, "images", f"sample_{label}.png"
    )  # pdf for vectorised images
