...
class DynamicVisionTransformer(nn.Module):
    def __init__(
        ...
        assert (
            d_model % n_heads == 0
        ), "d_model must be divisible by n_heads. Actually, I think we could relax this assumption, we'll need to adapt the code though..."
        ...
        self.transformer_encoders = (
            nn.ModuleList()
        )  # To get iterable (sequential isn't yet possible due to the masks).
