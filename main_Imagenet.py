...
for epoch in range(start_epoch + 1, epochs + 1):
    log.info(f"\nEpoch {epoch}/{epochs}")
    log.info(
        f"Epoch {epoch}: device={torch.device("cuda" if torch.cuda.is_available() else "cpu")}, cuda available={torch.cuda.is_available()}, GPU memory={torch.cuda.memory_allocated() / 1e6:.1f} MB"
    )
...
if cm_max is None:  # Test de la matrice de confusion car le rendu est bizarre
    log.warning(
        "Aucune matrice de confusion n'a été sauvée. Le modèle n'a pas validé ?"
    )
