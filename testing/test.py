...
def plot_performance_comparison(
    teacher_acc, student_acc, teacher_speed, student_speed, save_dir
):
...
def plot_per_class_accuracy(teacher_cm, student_cm, class_names, save_dir):
    ...
    rects1 = ax.bar(
        x - width / 2, teacher_cls_acc, width, label="Teacher", color="#1f77b4"
    )
    rects2 = ax.bar(
        x + width / 2, student_cls_acc, width, label="Student", color="#ff7f0e"
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(teacher_path):
        ...
        print(
            red(
                "Teacher checkpoint not found! Running with random weights (Results will be meaningless)."
            )
        )
    ...
    print(
        bold(
            f"student increased by {diff_speed}% the number of images processed per second."
        )
    )
