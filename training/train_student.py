...
if os.path.exists(teacher_checkpoint):
    ...
    raise FileNotFoundError(
        red(
            f"Teacher checkpoint not found at {teacher_checkpoint}. Please run run_teacher.py first!"
        )
    )
...
# Core Functions
def train_one_epoch(
    student, teacher, loader, optimizer, criterion, device, epoch_index
):
    student.train()
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        print(
            red(
                f"Epoch {epoch+1}/{epochs} | Loss: {train_loss:.4f} | Ratio loss: {ratio_loss:.4f} | Distill loss: {distill_loss:.4f} | kl loss : {kl_loss:.4f} | Train Acc: {train_acc:.2f}% | Val Acc: {val_acc:.2f}%"
            )
        )
        ...
        writer.add_scalar(
            "Student/LearningRate", optimizer.param_groups[0]["lr"], epoch
        )

        # Save Checkpoint
        if (epoch + 1) % 5 == 0:
            torch.save(
                student.state_dict(), f"{checkpoint_dir}/student_epoch_{epoch+1}.pth"
            )
    ...
    test_acc, cm = validate_one_epoch(
        student, test_loader, device, desc="Testing Student"
    )
