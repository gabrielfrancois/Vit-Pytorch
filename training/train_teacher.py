...
def save_training_plots(
    train_losses, val_losses, train_accs, val_accs, lrs, confusion_mat, save_dir
):
    """
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        print(
            red(
                f"Epoch {epoch+1}/{epochs} | Loss: {train_loss:.4f} | Train Acc: {train_acc:.2f}% | Val Acc: {val_acc:.2f}%"
            )
        )
        ...
        writer.add_scalar(
            "Teacher/LearningRate", optimizer.param_groups[0]["lr"], epoch
        )
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(
                teacher.state_dict(), f"{checkpoint_dir}/teacher_epoch_{epoch+1}.pth"
            )
